@props(['car' , 'isInWatchList'=>false, 'hide'=>null])

<div class="car-item card" {{$hide?" id=$hide " : ""}}>
    ...
    <div class="flex items-center justify-between">
        ...
        <button 
            class="btn-heart watchListBtn {{ $isInWatchList ? 'text-primary' : ''}}"
            value="{{auth()->check() ? auth()->user()->id : 'guest'}}"
            id="{{$car->id}}"
            onclick="watchlist({{$isInWatchList ? 1 : 0}} , {{auth()->check()}} ,{{auth()->check() ? auth()->user()->id : null }} , {{$car->id}})">
                <svg
